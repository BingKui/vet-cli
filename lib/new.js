...
function changePkgValue (filePath, answers, callback) {
    const isVite = answers.template == 'vite';
    fs.readFile(`${filePath}/package.json`, 'utf8', function (err, data) {
        const val = JSON.parse(data);
        val.name = answers.name;
        if (isVite) {
            fs.readFile(`${filePath}/electron-builder.json5`, 'utf8', function(err, data) {
                const val = JSON.parse(data);
                val.productName = answers.name;
                val.appId = answers.appid;
                val.copyright = answers.copyright;
                fs.writeFile(`${filePath}/electron-builder.json5`, JSON.stringify(val, null, '\t'), 'utf8', function (err) {});
            });
        } else {
